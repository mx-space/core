...
## Task 11: 封面抽屉去掉提示词并排，编译移入服务端

用户实测后的反馈：「点一下 AI 生成封面，然后它又有一个风格预设，又要让我写提示词，
这是在干嘛呀」。

病根不是多了个控件，是**把先后关系摆成了并列关系**。预设是一套编译规程（输入），
提示词是它的产物；二者同屏，读起来像要用户做两遍同一个决定。且一旦用户改了提示词，
预设的贡献早已烘进那段文字，此后再切 chip 只会冲掉修改——那排 chip 就成了谎。
加之当前只有一条预设，chip 行是「请从一个选项中选一个」。

### 目标形态

```
预设模式（默认）：选预设 → 生成。无提示词 UI，编译在服务端内部完成
手动模式：自己写提示词，与预设互斥
预设模式下另有「查看/编辑提示词」——点了才拟词并展开，展开即转入手动模式
```

**不要**把文章原文直接当提示词发给生图模型。生图模型吃长篇中文散文的效果很差，会去
逐个"画"文中的名词而非表达一个空间关系。九轴编译规程存在的意义正在于此。编译照做，
只是用户看不见。

### 1. 服务端：generate 接受预设模式

`GenerateImageSchema` 改为二选一：

- 手动：`prompt` 必填
- 预设：`presetId` + `refId` 必填，`prompt` 省略

用 `.refine()` 表达互斥，参照 `DraftImagePromptSchema` 已有的写法。

`ImageGenerationTaskPayload` 同步：`prompt?` / `presetId?` / `refId?`。

**编译放在 task handler 里，不放在 controller。** 入队接口须立即返回 taskId；若在
controller 里先跑一次文本模型再入队，用户点下生成到看见任务出现之间会空等数秒。

### 2. 拟词失败须降级，不得拖垮生图

handler 里若 `presetId` 模式而拟词失败（文本 provider 未配置、超时、模型报错），
**不要**让整个生成任务失败。降级为「文章标题 + 该预设的 `hardConstraints`」拼一个
最简提示词继续生成，并记一条 warn 说明降级原因。

理由：生图本身不需要文本模型。让一个可选的增强步骤成为必经的单点故障是错的。

### 3. admin 抽屉重构

- 默认预设模式：预设 chip 行 + 生成按钮，**无提示词 textarea**
- 「查看/编辑提示词」按钮：点击才调 `draft-prompt`，成功则展开 textarea 并转入手动
  模式（chip 变灰，注明已脱离预设）
- 手动模式可切回预设模式（清空 textarea）
- 预设只有一条时 chip 行仍显示（它现在是模式的标识，不再是"从一个选项中选一个"），
  但若日后只剩一条且用户从未切换，可考虑简化——本任务不做
- **`draft-prompt` 不再在开抽屉时自动调用**

### 4. 文本 provider 缺失的提示改在按钮上

`draft-prompt` 只在用户主动点「查看/编辑提示词」时才可能失败。彼时提示
「此功能需要文本 AI provider，去 AI 设置配置」正当其时——不再劈头盖脸砸在开抽屉那一刻。

（此项与前一轮派出的错误处理修复重叠，以本任务为准。）

### 边界

- 不动 `generate_image` agent tool（它本就是手写提示词，无预设）
- 不动去重、不动传输层
- 不新增数据库字段

### 验收

- 预设模式下 `POST /ai/image/generate` 不带 `prompt` 能成功入队并生成
- 拟词失败时任务仍完成，产出降级提示词的图，日志有 warn
- 手动模式行为不变
- 开抽屉不再触发 `draft-prompt` 请求
- 既有测试不回归
