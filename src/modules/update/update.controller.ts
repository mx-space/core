...
import { Observable, Subject, catchError, lastValueFrom, takeUntil } from 'rxjs'
...
export class UpdateController {
  ...
  async updateDashboard(
    ...
    // @ts-expect-error
    const sseOutput$ = new Observable<string>(async (observer) => {
      ...
      if (!isExistLocalAdmin) {
        ...
        const stream$ = this.service
          .downloadAdminAsset(currentVersion)
      ...
      const latestVersion = await this.service
        ...
        .catch((err) => {
          observer.next(
            chalk.red(`Fetching latest admin version error: ${err.message}`),
          )
          observer.complete()
          return ''
        })
      ...
      const stream$ = this.service
        .downloadAdminAsset(latestVersion)
        .pipe(takeUntil(new Subject()))
