...
/**
 * Schema migration test for 0011_enrichment_screenshots + 0014 capture
 * rename. Verifies the table exists under its renamed name with the expected
 * columns/types, the ON DELETE CASCADE link to enrichment_cache fires, and
 * the LRU index is present.
 */
describe('migration — enrichment_captures', () => {
  let context: PgTestDatabase

  beforeAll(async () => {
    context = await createPgTestDatabase('mx_enrichment_captures')
  }, 60_000)
  ...
  it('creates enrichment_captures with the spec column types', async () => {
    ...
    const { rows } = await context.pool.query(
      `select kcu.column_name
        ...
        where tc.table_schema = 'public'
          and tc.table_name = 'enrichment_captures'
    ...
    const { rows } = await context.pool.query(
      `select indexdef
        ...
        where schemaname = 'public'
          and tablename = 'enrichment_captures'
          and indexname = 'enrichment_captures_lru_idx'`,
    )
    expect(rows).toHaveLength(1)
    expect(rows[0].indexdef).toMatch(/last_accessed_at/i)
  })

  it('cascades deletes from enrichment_cache to enrichment_captures', async () => {
    ...
    const beforeRows = await context.pool.query(
      `select object_key, palette from enrichment_captures where enrichment_id = $1`,
      [enrichmentId],
    )
    ...
    const afterCount = await context.pool.query(
      `select count(*)::int as n from enrichment_captures where enrichment_id = $1`,
      [enrichmentId],
    )
