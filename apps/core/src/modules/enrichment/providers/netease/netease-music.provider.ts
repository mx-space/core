...
export class NeteaseMusicProvider implements EnrichmentProvider {
  ...
  isValidId(id: string): boolean { return /^\d+$/.test(id) }
  ...
  async fetch(id: string): Promise<EnrichmentResult> {
    ...
    if (song.artists?.length) attrs.push({ key: 'artist', value: song.artists.map((a: any) => a.name).join(', '), label: 'Artist', format: 'text' })
    if (song.album?.name) attrs.push({ key: 'album', value: song.album.name, label: 'Album', format: 'text' })
