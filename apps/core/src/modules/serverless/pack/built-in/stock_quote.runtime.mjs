export default async function handler(ctx) {
  ...
  const quote = await safeFetch('https://api.twelvedata.com/quote', {
    symbol: sym,
    apikey: apiKey,
  })
  ...
  let sparkline = []
  try {
    const series = await safeFetch('https://api.twelvedata.com/time_series', {
      symbol: sym,
      interval: '5min',
      outputsize: '78',
      apikey: apiKey,
  ...
  const asOf =
    quote.last_quote_at || quote.timestamp || Math.floor(Date.now() / 1000)
  ...
  const result = {
    ...
    fiftyTwoWeekHigh:
      num(quote.fifty_two_week && quote.fifty_two_week.high) ||
      num(quote.high),
    fiftyTwoWeekLow:
      num(quote.fifty_two_week && quote.fifty_two_week.low) || num(quote.low),
