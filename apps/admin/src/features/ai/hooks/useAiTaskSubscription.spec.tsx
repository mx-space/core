...
// Spec 2 step-27 — useAiTaskSubscription connection-fallback contract.
//
...
const { useAiTaskListSubscription, useAiTaskDetailSubscription } =
  await import('./useAiTaskSubscription')
...
function StatusProbe(props: {
  ...
  const result =
    props.scope === 'list'
      ? useAiTaskListSubscription()
      : useAiTaskDetailSubscription(props.taskId)
...
describe('useAiTaskListSubscription', () => {
...
describe('useAiTaskDetailSubscription', () => {
  ...
  it('reconnect invalidates the per-detail cache key only when a taskId is supplied', () => {
    ...
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: adminQueryKeys.ai.taskDetail('task-50'),
    })
  })
})
