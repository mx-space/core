...
export function DeskSkeletonBar(props: { className?: string }) {
  return (
    <div
      className={cn(
...
function SectionSkeleton(props: { lines: number }) {
  return (
    <div className="flex flex-col gap-3 border-t border-border py-7 phone:py-6">
      <DeskSkeletonBar className="h-3 w-24" />
      {Array.from({ length: props.lines }, (_, index) => (
        <DeskSkeletonBar className="h-4 w-full" key={index} />
      ))}
    </div>
  )
}
  ...
  return (
    <div aria-busy="true" className="flex flex-col phone:order-1">
      <div className="flex flex-col gap-3 pb-8 phone:pb-6">
        <DeskSkeletonBar className="h-3 w-48" />
        <DeskSkeletonBar className="h-8 w-64" />
        <DeskSkeletonBar className="h-4 w-80 max-w-full" />
      </div>
...
export function DeskRailSkeleton() {
  return (
    <div className="flex flex-col gap-3 py-6 phone:hidden">
      <DeskSkeletonBar className="h-3 w-16" />
      <DeskSkeletonBar className="h-8 w-full" />
      <DeskSkeletonBar className="mt-6 h-3 w-16" />
      <DeskSkeletonBar className="h-12 w-full" />
    </div>
  )
}
