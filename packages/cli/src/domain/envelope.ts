import { ValidationXml } from './errors'

// ---------------------------------------------------------------------------
// Port of `src/core/envelope.ts` — pure sync XML parser for `<mxpost>` /
// `<mxnote>` envelopes. Errors are thrown as `ValidationXml` (a
// `Data.TaggedError`); callers that want Effect ergonomics wrap with
// `Effect.try`/`Effect.sync` + `catch: (e) => e` (since the thrown value is
// already a CliError).
// ---------------------------------------------------------------------------
