...
import { publish } from '../../../src/cli/post/publish'
import { unpublish } from '../../../src/cli/post/unpublish'
...
describe('post publish / unpublish', () => {
  ...
  it('publish slug → resolves via get-url then PATCH', async () => {
    const http = testHttpLayer({
      'GET https://blog.example.com/api/v2/posts/get-url/hello-world': {
        status: 200,
        body: { path: '/writing/hello-world' },
      },
      'GET https://blog.example.com/api/v2/posts/writing/hello-world': {
        status: 200,
        body: { id: 'post-1' },
      },
