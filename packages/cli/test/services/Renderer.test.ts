...
describe('Renderer — emitPostList', () => {
  it.effect('emits pretty JSON by default', () =>
    withOpts(
      {},
      Effect.gen(function* () {
        const renderer = yield* Renderer
        yield* renderer.emitPostList({
    ...
    withOpts(
      ...
      Effect.zipRight(
        withOpts(
          ...
          Effect.gen(function* () {
            const renderer = yield* Renderer
            yield* renderer.emitPostList({ data: [] })
          }),
        ),
      ),
      Effect.tap(() =>
...
describe('Renderer — emitDocument', () => {
  it.effect('renders readable post metadata under --output readable', () =>
    withOpts(
      { output: 'readable' },
      Effect.gen(function* () {
        const renderer = yield* Renderer
        yield* renderer.emitDocument('post', {
          id: '1',
          title: 'Readable Post',
          slug: 'readable-post',
          isPublished: true,
      ...
      Effect.tap(() =>
        Effect.sync(() => {
          ...
          expect(out).toContain('title: Readable Post')
          expect(out).toContain('state: published')
          expect(out).toContain('category: Tech')
          expect(out).toContain('tags: cli, ai')
        }),
      ),
      Effect.provide(Renderer.Default),
    ),
  ...
  it.effect('renders note envelope under --output envelope', () =>
    withOpts(
      ...
      Effect.gen(function* () {
        const renderer = yield* Renderer
        yield* renderer.emitDocument('note', {
          title: 'Daily Note',
          slug: 'daily-note',
          is_published: false,
  ...
  it.effect('renders Lexical content as LiteXML in readable mode', () =>
    Effect.gen(function* () {
      ...
      const out = stdoutLines.join('')
      expect(out).toContain('content_format: litexml')
  ...
  it.effect('unwraps { data } documents and rejects unsupported output modes', () =>
    withOpts(
      ...
      Effect.gen(function* () {
        const renderer = yield* Renderer
        yield* renderer.emitDocument('page', {
          data: {
            id: 'pg1',
            title: 'Page',
            slug: 'page',
      ...
      Effect.zipRight(
        withOpts(
          ...
          Effect.gen(function* () {
            const renderer = yield* Renderer
            yield* renderer.emitDocument('post', { title: 'x' })
          }),
        ),
      ),
      Effect.tap(() =>
        Effect.sync(() => {
          ...
          expect(out).toContain('created_at: 2026-01-01T00:00:00.000Z')
          expect(out).toContain('content_format: text')
          expect(stderrLines.join('')).toContain(
            'unsupported --output value: table',
          )
        }),
      ),
      Effect.provide(Renderer.Default),
...
describe('Renderer — pure document renderers', () => {
  it('renders fallback lexical content, escaped envelope meta, and scalar variants', () => {
    const readable = renderReadableDocument('note', {
      id: 'n1',
      title: 'Note',
      slug: 'note',
      is_published: false,
    ...
    expect(readable).toContain('topic: life')
    expect(readable).toContain('weather: sun, wind')
    expect(readable).toContain('content_format: text')
    expect(readable).toContain('Plain fallback')

    const envelope = renderDocumentEnvelope('post', {
      title: 'A & B < C',
      slug: 'a-b',
      category: { id: 'cat-id' },
      isPublished: true,
