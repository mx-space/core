...
The CLI command surface includes `auth`, `post`, `note`, `page`, `category`, `topic`, `comment`, `snippet`, and `config`. Before using an unfamiliar command or flag, read [Command Index](references/command-index.md). Do not infer unsupported flags from another resource type.
